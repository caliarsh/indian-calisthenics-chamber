...
import { ContactAndConsentFields, CrmSubmitStatus, TurnstileField, useCrmSubmission } from '@/components/crm-form-fields';
import { buildLeadSubmission } from '@/lib/crm-client';
...
function validateContact(values: { phone: string; email: string }, consent: { enquiry: boolean }, token: string) {
  const errors: Errors = {};
  if (!validPhone(values.phone)) errors.phone = 'Enter a valid 10-digit Indian mobile number.';
  if (!validEmail(values.email)) errors.email = 'Enter a valid email address.';
  if (!consent.enquiry) errors.enquiryConsent = 'Please agree so ICC can respond to your enquiry.';
  if (process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY && !token) errors.turnstile = 'Please complete the security check.';
  return errors;
}
...
export function InternshipApplicationForm() {
  ...
  const crm = useCrmSubmission();

  async function submit(event: React.SyntheticEvent<HTMLFormElement>) {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    const fields = ['name', 'phone', 'email', 'city', 'background', 'experience', 'reason', 'availability'] as const;
    const values = Object.fromEntries(fields.map(field => [field, valueOf(form, field)])) as Record<(typeof fields)[number], string>;
    const next: Errors = validateContact(values, crm.consent, crm.turnstileToken);
    ...
    const message = [
      ...
      `Name: ${values.name}`, `Mobile: ${values.phone}`, ...(values.email ? [`Email: ${values.email}`] : []), `Preferred city: ${location.name}`, `Current background: ${values.background}`,
      `Training / coaching experience: ${values.experience}`, `Why I want to join: ${values.reason}`, `Availability: ${values.availability}`,
    ].join('\n');
    const url = `https://wa.me/${location.whatsappNumber}?text=${encodeURIComponent(message)}`;
    const lead = buildLeadSubmission({ kind: 'internship-application', name: values.name, phone: values.phone, email: values.email, enquiryConsent: crm.consent.enquiry, marketingConsent: crm.consent.marketing, details: { branch: location.id, background: values.background, experience: values.experience, reason: values.reason, availability: values.availability }, turnstileToken: crm.turnstileToken || 'configuration-pending' });
    const serverErrors = await crm.submit(lead, url); if (serverErrors) setErrors(current => ({ ...current, ...serverErrors }));
  }
  ...
  return <form className="trial-form community-form" onSubmit={submit} noValidate>
    ...
    <ContactAndConsentFields prefix="intern" errors={errors} consent={crm.consent} onConsentChange={crm.setConsent} />
    ...
    <TurnstileField onTokenChange={crm.setTurnstileToken} error={errors.turnstile} />
    <Button className="form-button" type="submit" disabled={crm.state.type === 'saving'}><MessageCircle aria-hidden="true" /> {crm.state.type === 'saving' ? 'Saving application…' : 'Save & apply on WhatsApp'} <ArrowUpRight aria-hidden="true" /></Button>
    <p className="form-note">We securely save your application for review, then prepare the WhatsApp message for your selected branch.</p><CrmSubmitStatus state={crm.state} whatsappUrl={crm.whatsappUrl} onRetry={crm.retry} />
  </form>;
}
...
export function AthleteApplicationForm() {
  ...
  async function submit(event: React.SyntheticEvent<HTMLFormElement>) {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    const fields = ['name', 'phone', 'email', 'city', 'discipline', 'level', 'competition', 'motivation', 'goal', 'instagram'] as const;
    const values = Object.fromEntries(fields.map(field => [field, valueOf(form, field)])) as Record<(typeof fields)[number], string>;
    const next: Errors = validateContact(values, crm.consent, crm.turnstileToken);
    if (!values.name) next.name = 'Please tell us your name.';
    ...
    const message = [
      ...
      `Name: ${values.name}`, `Mobile: ${values.phone}`, ...(values.email ? [`Email: ${values.email}`] : []), `Nearest branch: ${location.name}`, `Primary discipline: ${values.discipline}`, `Current level: ${values.level}`,
    ...
    const url = `https://wa.me/${location.whatsappNumber}?text=${encodeURIComponent(message)}`;
    const lead = buildLeadSubmission({ kind: 'athlete-application', name: values.name, phone: values.phone, email: values.email, enquiryConsent: crm.consent.enquiry, marketingConsent: crm.consent.marketing, details: { branch: location.id, discipline: values.discipline, level: values.level, competition: values.competition, motivation: values.motivation, goal: values.goal, instagram: values.instagram }, turnstileToken: crm.turnstileToken || 'configuration-pending' });
    const serverErrors = await crm.submit(lead, url); if (serverErrors) setErrors(current => ({ ...current, ...serverErrors }));
  }
  ...
  return <form className="trial-form community-form" onSubmit={submit} noValidate>
    ...
    <ContactAndConsentFields prefix="athlete" errors={errors} consent={crm.consent} onConsentChange={crm.setConsent} />
    ...
    <TurnstileField onTokenChange={crm.setTurnstileToken} error={errors.turnstile} />
    <Button className="form-button" type="submit" disabled={crm.state.type === 'saving'}><MessageCircle aria-hidden="true" /> {crm.state.type === 'saving' ? 'Saving application…' : 'Save & apply on WhatsApp'} <ArrowUpRight aria-hidden="true" /></Button>
    <p className="form-note">Applications are securely saved for review. Selection and competition support depend on readiness, conduct, event eligibility, and approval.</p><CrmSubmitStatus state={crm.state} whatsappUrl={crm.whatsappUrl} onRetry={crm.retry} />
  </form>;
}
...
export function SupportForm() {
  ...
  async function submit(event: React.SyntheticEvent<HTMLFormElement>) {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    const fields = ['name', 'phone', 'email', 'supportType', 'focus', 'range', 'message'] as const;
    const values = Object.fromEntries(fields.map(field => [field, valueOf(form, field)])) as Record<(typeof fields)[number], string>;
    const next: Errors = validateContact(values, crm.consent, crm.turnstileToken);
    ...
    const message = [
      ...
      `Name / organisation: ${values.name}`, `Mobile: ${values.phone}`, ...(values.email ? [`Email: ${values.email}`] : []), `Support type: ${values.supportType}`, `Preferred focus: ${values.focus}`,
      ...(values.range ? [`Indicative contribution: ${values.range}`] : []), `Message: ${values.message}`,
    ].join('\n');
    const url = `https://wa.me/${siteConfig.onlineWhatsappNumber}?text=${encodeURIComponent(message)}`;
    const lead = buildLeadSubmission({ kind: 'support-enquiry', name: values.name, phone: values.phone, email: values.email, enquiryConsent: crm.consent.enquiry, marketingConsent: crm.consent.marketing, details: { supportType: values.supportType, focus: values.focus, range: values.range, message: values.message }, turnstileToken: crm.turnstileToken || 'configuration-pending' });
    const serverErrors = await crm.submit(lead, url); if (serverErrors) setErrors(current => ({ ...current, ...serverErrors }));
  }
  ...
  return <form className="trial-form community-form" onSubmit={submit} noValidate>
    ...
    <ContactAndConsentFields prefix="support" errors={errors} consent={crm.consent} onConsentChange={crm.setConsent} />
    ...
    <TurnstileField onTokenChange={crm.setTurnstileToken} error={errors.turnstile} />
    <Button className="form-button" type="submit" disabled={crm.state.type === 'saving'}><MessageCircle aria-hidden="true" /> {crm.state.type === 'saving' ? 'Saving request…' : 'Save & discuss on WhatsApp'} <ArrowUpRight aria-hidden="true" /></Button>
    <p className="form-note">This website does not process contributions. Your request is saved for follow-up; verify the payment recipient directly with ICC.</p><CrmSubmitStatus state={crm.state} whatsappUrl={crm.whatsappUrl} onRetry={crm.retry} />
  </form>;
}
