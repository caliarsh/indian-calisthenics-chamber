...
export function TrialForm() {
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      if (siteConfig.locations.some(item => item.id === queryLocation)) setLocation(queryLocation);
  ...
  function handleSubmit(event: React.SyntheticEvent<HTMLFormElement>) {
    ...
    const values = { name: readText(form, 'name'), experience: readText(form, 'experience'), goal: readText(form, 'goal'), mode: readText(form, 'mode'), location: readText(form, 'location'), program: readText(form, 'program'), session: readText(form, 'session') };
    const selectedLocation = siteConfig.locations.find(item => item.id === values.location);
    ...
    if (!values.session) nextErrors.session = 'Choose a preferred time.';
    setErrors(nextErrors); setSent(false);
    if (Object.keys(nextErrors).length || !isMode(values.mode) || !option) return;
    const destination = values.mode === 'Offline' ? selectedLocation : siteConfig.locations.find(item => item.id === 'bengaluru');
    if (!destination) return;
    const message = [
      `Hi ${siteConfig.name}, I’d like to ${values.mode === 'Online' ? 'book a free 20-minute online consultation' : 'book an offline trial'}.`, '',
      `Name: ${values.name}`, `Level: ${values.experience}${assessed ? ' (website assessment)' : ''}`, `Training mode: ${values.mode}`,
      ...(selectedLocation ? [`Booking city: ${selectedLocation.name}`] : []), `Program / offer: ${option.label}`, `Training goal: ${values.goal}`, `Preferred time: ${values.session}`,
    ].join('\n');
    window.open(`https://wa.me/${destination.whatsappNumber}?text=${encodeURIComponent(message)}`, '_blank', 'noopener,noreferrer');
    setSent(true);
  }
  ...
  return <form className="trial-form" onSubmit={handleSubmit} noValidate>
    ...
    <div className="field"><label htmlFor="session">Preferred time</label><NativeSelect className="form-select" id="session" name="session" defaultValue="" aria-invalid={Boolean(errors.session)} aria-describedby={errors.session ? 'session-error' : undefined}><NativeSelectOption value="" disabled>Choose a time</NativeSelectOption>{(mode === 'Online' ? ['Weekday morning','Weekday afternoon','Weekday evening','Weekend'] : ['Weekday morning','Weekday evening','By appointment']).map(value => <NativeSelectOption value={value} key={value}>{value}</NativeSelectOption>)}</NativeSelect>{errors.session && <p className="field-error" id="session-error">{errors.session}</p>}</div>
